...
public class Condition2Test {
	 ...
	 * implementation uses several condition variables, and some "tricks" to
	 * synchronize everything at the end of the execution. This could be done
	 * better with a join() call to wait for threads to finish. But at the
	 * moment, the main thread waits to be signaled that all producers and
	 * consumers have exited. This demonstrate that synchronization primitives
	 * can be built on top of others, or each other.
	 */
	private static class ProdConsBuffer {

		/**
		 * Constructor: takes as parameter the maximum number of actions (i.e.,
		 * producing or consuming an item) before all producers and consumers
		 * call it quit. This is to avoid an infinite execution.
		 */
		public ProdConsBuffer(int maxNumActions) {
			...
			this.rng = new Random(1024);
		}

		/**
		 * Method to consume an item. This method is NOT thread-safe and should
		 * by called within a critical section via this.mutex. This function
		 * should never be called on an empty buffer.
		 */
		public int consumeItem() {
			...
			if (lastItemIndex == -1) {
				System.out
						.println("Error: Can't consume item because buffer is empty!!");
				return -1;
			}
		...
		/**
		 * Method to produce an item. This method is NOT thread-safe and should
		 * by called within a critical section via this.mutex. This function
		 * should never be called on a full buffer.
		 */
		public void produceItem(int item) {
			...
			if (lastItemIndex == maxNumItems - 1) {
				System.out
						.println("Error: Can't produce item because buffer is full!!");
				return;
			}
			...
			if (buffer[lastItemIndex + 1] != -1) {
				System.out
						.println("Error: Produced item at an invalid position!!");
				return;
			}
		...
		/* Condition variables */
		public Condition2 isNotEmptyCond; /*
										 * signaled when the buffer becomes
										 * non-empty
										 */
		public Condition2 isNotFullCond; /*
										 * signaled when the buffer becomes
										 * non-full
										 */
	...
	/**
	 * Producer class, which implements a producer thread that puts data in a
	 * buffer.
	 */
	private static class Producer implements Runnable {
		...
		public void run() {
			...
			while (true) {
				...
				/*
				 * If the buffer is full, wait on isNotFullCond. This is in a
				...
				while (!buffer.isDone && buffer.isFull) {
					...
					buffer.isNotFullCond.sleep(); /*
												 ...
												 * reacquires it when it wakes
												 * up
												 */
				}

				/*
				 * I just woke up, and perhaps it's because it's all over in
				 * which case I exit from my main loop
				 */
				if (buffer.isDone) {
	...
	private static class Consumer implements Runnable {
		...
		public void run() {
			...
			while (true) {
				...
				buffer.mutex.acquire();
				/*
				 * If the buffer is empty, wait on isNotEmptyCond. This is in a
				 * while loop to avoid spurious wake-ups
				 */
	...
	public static void runTest() {
		...
		for (int i = 0; i < numProducers; i++) {
			producers[i] = new KThread(new Producer(i, buffer))
					.setName("producer thread #" + i);
			producers[i].fork();
		}
		...
		for (int i = 0; i < numConsumers; i++) {
			consumers[i] = new KThread(new Consumer(i, buffer))
					.setName("consumer thread #" + i);
			consumers[i].fork();
		}
